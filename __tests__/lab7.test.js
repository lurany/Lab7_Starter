describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (let i = 0; i < prodItemsData.length; i++) {
      ...
      const productItem = prodItemsData[i];
    // Make sure the title, price, and image are populated in the JSON
      if (productItem.title.length == 0) { allArePopulated = false; }
      if (productItem.price.length == 0) { allArePopulated = false; }
      if (productItem.image.length == 0) { allArePopulated = false; }
    }
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i = 0; i < productItems.length; i++) {
      await productItems[i].evaluate(productItem => {
    ...
    const cartCount = await page.$eval('#cart-count', cartCountElement => {
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const productItems = await page.$$('product-item');
    for (let i = 0; i < productItems.length; i++) {
      const buttonText = await productItems[i].evaluate(productItem => {
        const button = productItem.shadowRoot.querySelector('button');
